...
for (iloop in 1:8) {
  if (doloop[iloop] == T) {
     ...
     tsys := [100.0,  720.0]
     doif[1:4] := T
        dolist := spaste('cd ',sdf_dir,' && ls -d *IF',iloop,'.sdfits',' && ls -d *IF',iloop2,'.sdfits')

     }
  if (iloop == 5) 
     {
     tsys := [100.0,  700.0]
     dolist := spaste('cd ',sdf_dir,' && ls -d *IF',iloop,'.sdfits')
     doif[iloop] := T
     }
  if (iloop > 5) 
     {
     tsys := [300.0,  1000.0]
     dolist := spaste('cd ',sdf_dir,' && ls -d *IF',iloop2,'.sdfits',' && ls -d *IF',iloop,'.sdfits', ' && ls -d *IF',iloop3,'.sdfits')
     doif[6:8] := T
     }
  endif
