...
int main(int argc, char **argv){
  ...
  if(proc_id==0){
    ...
    sprintf(command,"cp %smasterquickldata7mm.g %sldata.g",getenv("MOPRAGLISHSCRIPT"),folderoutputname);
    system(command);
    sprintf(command,"ls  %s*.rpf | wc -l >tmp.txt",foldertotalinputname); /*  count the number of .rpf files*/
    system(command);
  
    sprintf(command,"(cd %s; ls *.rpf) >>tmp.txt",foldertotalinputname); /*add the list of files into this tmp dump */
    system(command);
  ...
  for (i=0; i<number_of_files;i++){
    for (j=0;j<task_per_file;j++){
      ...
      sprintf(command, "sed 's/ifno in 1:16/ifno in %d:%d/g' %s>tmpldata.g; mv tmpldata.g %s ",1+16/task_per_file*j,16/task_per_file*(j+1), procedure_filename, procedure_filename);
